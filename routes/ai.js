...
const db = require('../db');
const { USE_POINTS } = require('./usage');
...
async function checkUserAccess(userId) {
  ...
  if (user.points >= USE_POINTS_LOCAL) return { canUse: true, source: 'points' };

  return { canUse: false, message: '免费次数已用完且积分不足，请购买积分或会员', points: user.points, needPoints: USE_POINTS_LOCAL };
}
...
router.post('/chat', async (req, res) => {
  try {
    ...
    if (!DEEPSEEK_API_KEY) {
      return res.json({ ok: false, message: 'DeepSeek API Key 未配置，请联系管理员' });
    }
